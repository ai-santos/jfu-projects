...
var projTemplate = _.template('<% _.each(projects, function(project, index, projects) { %>' + '<div class="col-md-3">' + '<h5><%= project.project_name %></h5>' + '<h5><%= project.address %></h5>' + '<h5><%= project.city %></h5>' + '<h5><%= project.state %></h5>' + '<h5><%= project.zip %></h5>' + '<h5><%= project.phone %></h5>' + '<h5><%= project.contact %></h5>' + '<h5><%= project.email %></h5>' + '</div>' + '<% }); %>');

var content = projTemplate({
  projects: projects
});

var proj_container = document.querySelector('.projects');
